...
async function handleMessage(sender_psid, received_message) {
  ...
  if (received_message.text) {
    ...
    if (received_message.text.indexOf("Registra") !== -1) {
      ...
      // Create the payload for a basic text message
      response = {
        text: `🌇 Adesso inviami il nome della città della tua attività - SOLO il nome della città`
      };
      state[sender_psid] = 2;
      console.log(state);
      ...
      response = {
        text: `📍 Bene! Adesso inviami il CAP dove si trova la tua attività - SOLO il CAP`
      };
...
function handlePostback(sender_psid, received_postback) {
  ...
  // Set the response based on the postback payload
    ...
    response = {
      text: `🏠 Bene! Adesso inviami il nome della via della tua attività (Es: Corso Italia 11, Viale Garibaldi 12, ...) - SOLO il nome della via e numero civico`
    };
...
function callMap(sender_psid, address, city, cap) {
  var coordinates;
  console.log(
    "https://nominatim.openstreetmap.org/search?street=" +
      address + "&city=" + city + "&cap=" + cap + "&format=json"
      // "&key=" +
      // process.env.MAP_TOKEN
  );
  request(
    ...
    (err, res, body) => {
      if (!err) {
        ...
        if (body.length == 0) {
          ...
          coordinates =
            body[0]["lat"] + body[0]["lon"]
          // Create the payload for a basic text message

          response = send_confirmation(address+","+city+","+cap, coordinates);
          //state[sender_psid] = -1;
          // Sends the response message
        }
        callSendAPI(sender_psid, response);
